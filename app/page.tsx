...
function Blobs() {
  const blobs = [
    { color: '#FF6B6B', x: 8, y: 15, size: 56, delay: 0 },
    { color: '#4ECDC4', x: 15, y: 60, size: 40, delay: 1.2 },
    { color: '#7EB8FF', x: 75, y: 20, size: 64, delay: 0.5 },
    { color: '#A855F7', x: 85, y: 65, size: 48, delay: 2 },
    { color: '#F7C948', x: 90, y: 85, size: 36, delay: 0.8 },
    { color: '#FF9F43', x: 30, y: 80, size: 44, delay: 1.5 },
    { color: '#48DBFB', x: 60, y: 75, size: 32, delay: 0.3 },
    { color: '#FF6B9D', x: 45, y: 10, size: 28, delay: 1.8 },
    { color: '#26de81', x: 5, y: 85, size: 36, delay: 2.4 },
    { color: '#fd9644', x: 92, y: 40, size: 30, delay: 1.1 },
  ]
    ...
    <div style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 0, overflow: 'hidden' }}>
      {blobs.map((b, i) => (
        <div key={i} style={{
          ...
          filter: 'blur(2px)',
          opacity: 0.7,
          animation: `float${i % 3} ${6 + b.delay}s ease-in-out infinite`,
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .skel {
          ...
          background: linear-gradient(90deg, #1a1a22 25%, #1f1f2a 50%, #1a1a22 75%);
        ...
        .t-card:nth-child(1){animation-delay:.04s}
        .t-card:nth-child(2){animation-delay:.08s}
        .t-card:nth-child(3){animation-delay:.12s}
        .t-card:nth-child(4){animation-delay:.16s}
        .t-card:nth-child(5){animation-delay:.20s}
        ...
        <div className="content">
          ...
          <div className="stats-row">
            {[
              ...
              { label: 'Gainers', val: loading ? '—' : `${gainersCount} / ${tokens.length}`, change: null },
              { label: 'Top Gainer', val: loading ? '—' : topGainer ? `${topGainer.symbol.toUpperCase()}` : '—', change: loading ? null : topGainer ? `+${topGainer.price_change_percentage_24h.toFixed(1)}%` : null },
            ].map((s, i) => (
          ...
          {tab === 'tokens' && (
            <>
              ...
              <div className="foot">Data by CoinGecko · updates every 60s</div>
            </>
          )}
                ...
                {loading
                  ...
                  : trending.map(t => (
                    <div key={t.id} className="t-card" onClick={() => window.location.href = `/token/${t.id}`}>
                      ...
                      <div className={`t-card-chg chg-up`}>+{t.price_change_percentage_24h.toFixed(2)}%</div>
                    </div>
                  ))
                }
              </div>
